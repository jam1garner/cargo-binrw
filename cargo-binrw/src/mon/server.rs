...
pub(crate) struct RelayServer<T: BinrwStateMachine + Sized, U: Tracer + Sized + Send> {
...
impl RelayServer {
    /// spawn
    ///
    /// Binds `host`:`port`, unless it can't because the resource
    /// is not available.
    ///
    /// Should 
    pub fn spawn(host: Into<::std::net::IpAddr>, port: u16) -> BinResult<RelayServer, &'static str> {
        host.into::<::std::net::IpAddr>()
